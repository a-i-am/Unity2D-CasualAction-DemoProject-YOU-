...
namespace Assets
{
    public class PlayerScr : Singleton<PlayerScr>
    {
        ...
        private bool TryDie()
        {
            if (health.PlayerCurrentVal > 0f) return false;
        ...
        public void RespawnNearSafePosition()
        {
            StopAllCoroutines();
            ...
            transform.position = lastSafePosition + Vector3.up * 0.25f;

            Physics2D.IgnoreLayerCollision(6, 7, false);
            spriteRenderer.color = new Color(1, 1, 1, 1);
            playerAnimScr.DeadJumpAnimation(false);
            health.PlayerCurrentVal = health.PlayerMaxVal;
        }
    }
}
